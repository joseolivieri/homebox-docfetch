...
| D26 | **SQLite event store replaces the notes bus** (plan-architecture-v2 M2). Append-only `events` table; qr/approve/reject are deduped *signal* events the scanner acts on (never pruned); everything else is audit history (pruned after `notes.event_retention_days`, default 365). Portal writes signals directly to the shared store and `trigger`s immediate scanner processing (the DB bus doesn't bump `updatedAt`, so the change-poll can't see it). Entity notes reduced to a one-line breadcrumb (`docfetch: manual ✓ … — [log](…)`, `notes.breadcrumb`, default on). Change-poll cursor persisted in a `meta` table — restarts no longer eat pending change signals. Log surfaces: portal `/log` + `/log/{id}` (server-rendered HTML) and `docfetch log` CLI. Transition: split-mode portal still writes legacy notes lines (`legacyNotes`); the scanner imports semantic notes lines into events idempotently on every process — both die at M3. Supersedes D18/D19's notes-as-bus mechanics; the D20 egress boundary is unchanged. **Tradeoff accepted:** rejection memory now lives in the state volume (back it up), not in Homebox. | The notes bus hit structural limits: ~1000-char server cap forced budget/prune, oversized PUTs 500'd away whole payloads, every own-write self-triggered the change-poll, and semantic lines were regex-parsed prose. One process (D25) makes a single-writer store the correct bus. |
