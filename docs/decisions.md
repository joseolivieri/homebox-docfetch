...
| D26 | **SQLite event store replaces the notes bus** (plan-architecture-v2 M2). Append-only `events` table; qr/approve/reject are deduped *signal* events the scanner acts on (never pruned); everything else is audit history (pruned after `notes.event_retention_days`, default 365). Portal writes signals directly to the shared store and `trigger`s immediate scanner processing (the DB bus doesn't bump `updatedAt`, so the change-poll can't see it). Entity notes reduced to a one-line breadcrumb (`docfetch: manual ✓ … — [log](…)`, `notes.breadcrumb`, default on). Change-poll cursor persisted in a `meta` table — restarts no longer eat pending change signals. Log surfaces: portal `/log` + `/log/{id}` (server-rendered HTML) and `docfetch log` CLI. No migration/compat shims: pre-M2 collections are reset, not imported (user call, 2026-07-16). Supersedes D18/D19's notes-as-bus mechanics; the D20 egress boundary is unchanged. **Tradeoff accepted:** rejection memory now lives in the state volume (back it up), not in Homebox. | The notes bus hit structural limits: ~1000-char server cap forced budget/prune, oversized PUTs 500'd away whole payloads, every own-write self-triggered the change-poll, and semantic lines were regex-parsed prose. One process (D25) makes a single-writer store the correct bus. |
| D27 | **No aggregation jobs.** Events are written synchronously at each decision point; aggregation is read-time SQL. The only periodic maintenance is retention pruning in the weekly reconcile. | Batch aggregation would add drift (two sources of truth) and a scheduling surface for zero benefit at homelab data volume. |
| D28 | **Provenance outranks format; some artifacts are links, not files.** Two halves. (a) When no official file exists for a class but the maker has a page matched to the model, link the page rather than attach a third-party PDF — extends D24's official-first across formats. A rehost of the RIGHT product passes content skimming perfectly (it carries exactly the content skim checks for), so no amount of reading bytes separates a copy from the original; official provenance is the one signal a rehost cannot fake. `ModelMatch` required, so a generic brand support page cannot displace a real document; an official PDF still wins outright. (b) Doc classes gain `kind: file|link`. A link class writes an official URL into its field and does nothing else — no download, no skim, no attachment. New defaults: `product`, `specs` (all items) and `downloads` (category-gated to computer/network hardware). Gate is official-domain AND model-matched. | Observed live: a USW-Flex-2.5G-5 got an Australian reseller's HTML-to-PDF conversion of Ubiquiti's own page as its "manual", while the real install guide sat on ui.com unlinked. Ubiquiti — like much of modern hardware — publishes no manual PDF, so every PDF in existence for that switch is somebody's copy. Forcing a URL-shaped artifact through the file path is what makes the pipeline settle for a rehost. Firmware/driver indexes are also the page an owner actually returns to years later, and had no home in a file-only model. |
